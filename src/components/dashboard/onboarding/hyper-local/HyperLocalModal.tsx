...
export function HyperLocalModal({
  ...
  const viewOnMap = mapEnabled ? openMap : undefined;
        ...
        <div className="px-5 py-4 sm:px-6 sm:py-5 border-b border-slate-100 bg-gradient-to-r from-blue-50/80 via-indigo-50/50 to-white flex items-center justify-between shrink-0">
          ...
          {mapEnabled && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white border border-blue-200 text-blue-700 text-xs font-bold hover:bg-blue-50 shrink-0"
            >
                          ...
                          {metroPills.map((pill) => {
                            ...
                            return viewOnMap ? (
                              <button
                                key={pill.mode}
                                type="button"
                                onClick={() => openMap(transit.nearestMetro || null, pill.mode)}
                              ...
                              <span key={pill.mode} className={cls}>
                                <Icon className="w-3 h-3" />
                                {pill.label}
                              </span>
                            );
