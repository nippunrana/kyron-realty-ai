...
export function ImageUploadModal({
  ...
  // Build standard message for WhatsApp / share
  const buildShareMessage = () => {
    return buildPhotoUploadShareMessage(uploadUrl, propertyTitle);
  };

  // Copy full formatted share message
  ...
  const handleDownloadQr = () => {
    ...
    a.href = url;
    a.download = `kyron-upload-qr-${draftId || "property"}.svg`;
  ...
  // Share to WhatsApp (Web Share API with image file if supported, fallback to WhatsApp URL)
  const handleShareToWhatsApp = async () => {
    setIsSharing(true);
    const messageText = buildShareMessage();
    const whatsAppUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(messageText)}`;

    try {
      // If Web Share API with file support is available on this browser/OS
      if (typeof navigator !== "undefined" && navigator.share && qrCodeSvg) {
        try {
          const pngBlob = await svgToPngBlob(qrCodeSvg);
          const file = new File([pngBlob], `kyron-upload-qr-${draftId || "property"}.png`, {
            type: "image/png",
          });

          if (navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
              title: `Upload Photos: ${propertyTitle || "Property"}`,
              text: messageText,
              files: [file],
            });
            setIsSharing(false);
          ...
          console.warn("Native file share failed, falling back to WhatsApp Web:", shareErr);
        }
      }

      // WhatsApp Web / App intent URL fallback
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-slate-50 to-blue-50/40 border border-slate-200 flex flex-col sm:flex-row items-center gap-5">
              ...
              <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm shrink-0 flex flex-col items-center">
                ...
                <button
                  ...
                  className="mt-2 inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 border border-slate-200/80 rounded-lg transition-colors cursor-pointer"
                  title="Download vector QR code (SVG)"
                >
              ...
              <div className="flex-1 min-w-0 space-y-3 text-left w-full">
                <div>
                  ...
                  <p className="text-xs text-slate-600 mt-1">
                    1. Open phone camera and point at the QR code, or share via WhatsApp to a collaborator.
                    <br />
                    2. The mobile uploader opens instantly (no login or app download required).
                    <br />
                    3. Snap or pick photos — they appear live on this screen!
                  </p>
                </div>

                {/* Primary WhatsApp Share CTA Row */}
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  <button
                    type="button"
                    onClick={handleShareToWhatsApp}
                    disabled={isSharing}
                    className="flex-1 py-2.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white text-xs font-bold transition-all shadow-md shadow-emerald-600/20 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-70"
                  >
                    <WhatsAppIcon className="w-4 h-4" />
                    <span>{isSharing ? "Preparing Share..." : "Share to WhatsApp"}</span>
                  </button>

                  <button
                    type="button"
                    onClick={handleCopyMessage}
                    className="py-2.5 px-3 rounded-xl border border-slate-200 hover:bg-slate-100 text-slate-700 text-xs font-semibold transition-all flex items-center justify-center gap-1.5 cursor-pointer shrink-0"
                    title="Copy formatted message with upload link & QR info"
                  >
                    {copiedMessage ? (
                      <>
