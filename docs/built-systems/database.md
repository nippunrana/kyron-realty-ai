...
- **Never hand-run a migration against production.** `scripts/deploy.sh` applies migrations automatically on every deploy. Generate locally with `npm run db:generate` and commit the result; CI does the rest. A failed migration aborts the deploy; never add a fallback that lets the build continue past it, because the app would then run against a schema it does not match.
...
- **Properties table is the single source of truth for listing knowledge and negotiation rules.** `property_knowledge_bases` and `negotiation_matrices` were consolidated into `properties.knowledge_base` and `properties.negotiation_rules` (JSONB) on 2026-09-09. Never re-introduce separate 1:1 child tables or duplicate search columns (`city`, `state`, `listing_type`, `price`).
