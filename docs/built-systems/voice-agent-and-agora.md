...
- **Cloud Gateway REST calls authenticate via dynamic Token Auth (`agora token=...`)**: AccessToken2 combined tokens are minted on the fly from `AGORA_APP_ID` and `AGORA_APP_CERTIFICATE` for all REST endpoints (`/join`, `/update`, `/leave`, telemetry). `AGORA_CONVERSATIONAL_AI_API_KEY`, `AGORA_CUSTOMER_ID/SECRET`, and `NEXT_PUBLIC_AGORA_APP_ID` are not required in `.env` (the browser receives `appId` in the session start response).
