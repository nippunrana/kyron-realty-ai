...
- **QR Code SVGs must specify explicit dimensions (`width: 256`) and flex containers must use `[&>svg]:w-full [&>svg]:h-full`.** SVGs without explicit width/height collapse to 0x0 inside centered flexbox containers in Safari/WebKit.
...
- **Never select the full `properties` row for a public page, and never hand the raw `knowledge_base` JSONB to a client component.** The slug page's row is serialized into the client component's props, so `upload_token`, `owner_id`, `negotiation_rules` and the agent script would ship to every visitor. Pick public columns and buyer-facing knowledge fields explicitly in `src/app/listings/[slug]/`.
